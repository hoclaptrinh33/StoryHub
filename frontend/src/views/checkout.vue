      ...
      <div v-else class="sales-grid-layout">
        <section class="card-shell customer-column">
          ...
          <div class="customer-smart-wrap" :class="{ alert: showCustomerWarning }">
            ...
            <div
              v-if="customerQuery.trim().length > 0 && customerMatches.length === 0 && !customerObj"
              class="quick-create-wrap"
            >
              <p>Chưa có khách hàng này. Thêm mới:</p>
              <div class="inline-customer-form">
                <input v-model="newCustomerName" type="text" placeholder="Tên khách hàng (*)" class="form-input" />
                <input v-model="newCustomerPhone" type="text" placeholder="Số điện thoại (*)" class="form-input" />
                <input v-model="newCustomerAddress" type="text" placeholder="Địa chỉ (không bắt buộc)" class="form-input" />
              </div>
              <button
                class="btn-secondary mt-2"
                type="button"
                :disabled="isCreatingCustomer || !newCustomerName.trim() || newCustomerPhone.replace(/\D/g, '').length < 9"
                @click="createAndSelectCustomer"
              >
                <span class="material-icons">person_add</span>
                {{ isCreatingCustomer ? "Đang lưu..." : "Lưu khách hàng mới" }}
              </button>
            </div>
            ...
            <table>
              ...
              <tbody>
                ...
                >
                  ...
                  <td>{{ formatCurrency(item.type === 'rental' ? (item.rent_price || item.price) : item.price) }}</td>
                  <td>
...
const rentalsTotal = computed(() => cart.value.filter((item) => item.type === "rental").reduce((acc, item) => acc + (item.rent_price || item.price), 0));
const depositTotal = computed(() => cart.value.filter((item) => item.type === "rental").reduce((acc, item) => acc + (item.deposit_price !== undefined && item.deposit_price !== null ? item.deposit_price : computeDeposit(item.price)), 0));
...
const createAndSelectCustomer = async () => {
  if (!newCustomerName.value.trim() || newCustomerPhone.value.replace(/\D/g, "").length < 9) {
    return;
  }

  isCreatingCustomer.value = true;
  try {
    const created = await upsertCustomer(newCustomerPhone.value.replace(/\D/g, ""), {
...
const addToCart = async (product: InventoryItemListItem | undefined, source: "scanner" | "manual") => {
  ...
  if (product.type === "rental" && !customerObj.value) {
    setTimeout(() => {
...
watch(customerQuery, (value) => {
  ...
  if (!customerObj.value) {
    ...
    const namePart = trimmed.replace(/\d/g, "").trim();
    if (namePart) newCustomerName.value = namePart;
    if (digits) newCustomerPhone.value = digits;
  }
  ...
  const exactMatch = customerMatches.value.find((c) => c.name.toLowerCase() === trimmed.toLowerCase());
  
  if (exactMatch) {
    selectCustomer(exactMatch, true);
  } else if (customerMatches.value.length === 1 && trimmed.length >= 4) {
