...
SELECT id, volume_id, condition_level, 
       case when status = 'sold' then 'available' else status end, 
       health_percent, notes, reserved_by_customer_id, reserved_at, reservation_expire_at, version_no, item_type, created_at, updated_at, deleted_at FROM item;
